    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            if (code.equals(account.getCode())) {
                ...
                List<JobSeeker> jobSeekerList = jobSeekerDAO.getAllJobSeekers();
                JobSeeker jobSeeker = new JobSeeker();
                jobSeeker.setId(jobSeekerList.size() + 1);
                jobSeeker.setImageUrl(pictureUrl);
                jobSeeker.setName(userName);
                boolean genderValue = Boolean.parseBoolean(gender);
                jobSeeker.setGender(genderValue);
                jobSeeker.setEmailContact(userEmail);
                Account accountFromSession = (Account) session.getAttribute("account");
                jobSeeker.setAccountId(accountFromSession.getUsername());
